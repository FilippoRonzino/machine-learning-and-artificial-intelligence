...
def create_ecg_datasets(train_patients: list, test_patients: list, channel: int, downsample_factor: int, 
                        n_train_segments: int, n_test_segments: int, sample_length: int = 80, 
                        threshold: float = 20, max_attempts: int = 10000) -> tuple[np.ndarray, np.ndarray]:
    """
    Create training and testing datasets from ECG signals. It needs in the folder data/ecg_data the ecg data from PhysioNet.
    ...
    :param channel: Index of the channel to extract from each record.
    :param downsample_factor: Factor by which to downsample each signal.
    :param n_train_segments: Number of segments to sample for training.
    :param n_test_segments: Number of segments to sample for testing.
    :param sample_length: Length of each segment to sample.
    :param threshold: Minimum sum of absolute values required for a segment to be considered active.
    :param max_attempts: Maximum number of sampling attempts before giving up.
    :return: train_segments: Array of sampled signal segments for training.
             test_segments: Array of sampled signal segments for testing.
    """
